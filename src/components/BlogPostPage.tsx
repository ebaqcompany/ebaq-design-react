...
const SelectedWork = ({ selectedWorkSlugs = [] }: { selectedWorkSlugs?: string[] }) => {
  ...
  return <section className="selected-work-carousel border-y border-black/10 px-[5%] py-10 md:py-14" aria-label="Selected work" onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)} onFocus={() => setPaused(true)} onBlur={() => setPaused(false)}><div className="container"><div className="flex items-end justify-between gap-4"><div><p className="mb-3 font-semibold text-brand-primary md:mb-4">Selected work</p><h2 className="mb-0">Brand identities and websites</h2></div></div><div className="mt-8 overflow-hidden" onPointerDown={(event) => { if (event.pointerType !== "mouse" || event.button === 0) dragStart.current = event.clientX; }} onPointerUp={(event) => finishDrag(event.clientX)} onPointerCancel={() => { dragStart.current = null; }}><div onClickCapture={(event) => { if (dragged.current) { event.preventDefault(); event.stopPropagation(); dragged.current = false; } }}><Portfolio16Project {...activeProject!} reverse={false} /></div></div><div className="mt-8 flex items-end justify-end gap-2 md:ml-[calc(50%+3rem)] md:justify-start md:gap-4"><TestimonialCarouselArrow direction="previous" label="Previous case study" onClick={() => navigate(-1)} /><TestimonialCarouselArrow direction="next" label="Next case study" onClick={() => navigate(1)} /></div><a href="/work" className="button-primary inline-flex items-center gap-2 mt-6 md:mt-8 px-5 py-2.5 rounded-full border transition-colors duration-200">See all case studies</a></div></section>;
};
...
const buildLogoRosterBody = (roster: string[], pageImages: Array<Array<{ src: string; alt: string }>> = [], tileImages: Array<{ src: string; alt: string }> = [], research: LogoResearch[] = []) => {
  ...
  let html = '<h2 id="ninety-nine-examples">99 Best Law Firm Logos in 2026</h2><p>Here is the complete 01–99 roster from the current LAW FIRM LOGOS board.</p><p>I am reviewing each mark for clarity, confidence, distinctiveness, and how well its visual logic can extend into a professional identity system.</p><p><strong>Editorial note:</strong> the live board contains exact duplicates at 50 and 60, Allen &amp; Gledhill, and at 51 and 63, Rajah &amp; Tann Asia.</p><p>I have preserved the authoritative order and count rather than silently replacing either entry.</p>';
  ...
  html = html.replace(/<h2 id="ninety-nine-examples">99 Best Law Firm Logos in 2026<\/h2><p>Here is the complete 01–99 roster from the current LAW FIRM LOGOS board\.<\/p><p>I am reviewing each mark for clarity, confidence, distinctiveness, and how well its visual logic can extend into a professional identity system\.<\/p><p><strong>Editorial note:<\/strong> the live board contains exact duplicates at 50 and 60, Allen &amp; Gledhill, and at 51 and 63, Rajah &amp; Tann Asia\.<\/p><p>I have preserved the authoritative order and count rather than silently replacing either entry\.<\/p>/, "");
