...
import { founderDecisionContent } from "../content/founderDecisionContent";
...
export const Layout485 = (props: Layout485Props) => {
  ...
  return (
    <section id="relume" ref={ref} className="bg-[#f5f5f5] px-[5%] py-16 md:py-24 lg:py-28">
        ...
        {feature.buttons.map((button, index) => (
          button.variant === "link" ? (
            <a
              key={index}
              href="#pricing"
              className="inline-flex items-center gap-1 text-base font-medium transition-colors hover:text-[#00afec]"
            >
...
export const Layout485Defaults: Props = {
  features: founderDecisionContent.facts.map((fact, index) => ({
    number: String(index + 1).padStart(2, "0"),
    tagline: fact.label,
    heading: fact.heading,
    description: fact.value,
    buttons: [
      { title: "Book a Call", onClick: scrollToBook },
      {
        title: "See Pricing",
        variant: "link",
        size: "link",
